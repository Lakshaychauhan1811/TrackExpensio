...
try:
    ...
    RAG_ML_AVAILABLE = True
except ImportError:
    faiss = None  # type: ignore
    SentenceTransformer = None  # type: ignore
    RAG_ML_AVAILABLE = False
...
async def _groq_extract_from_text(raw_text: str) -> Dict[str, Any]:
    """Extract receipt fields via Groq when FAISS/sentence-transformers are unavailable."""
    _require_env("GROQ_API_KEY")
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    snippet = raw_text[:8000]
    groq_client = Groq(api_key=os.getenv("GROQ_API_KEY"))
    prompt = f"""Extract bill/receipt information from this document text. Return JSON only.

Document:
{snippet}

Fields: merchant, amount (number), currency, category, date (YYYY-MM-DD), notes.
If date missing use {today}. Default currency INR."""
...
async def run_receipt_rag_pipeline(file_path: str) -> Dict[str, Any]:
    ...
    if not RAG_ML_AVAILABLE:
        return await _groq_extract_from_text(raw_text)
